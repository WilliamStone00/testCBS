...
namespace CBS.FrontDesk.UI.Controllers.Accounting
{
    ...
    public class CashFlowManagementController : BaseController
    {
        ...
        [HttpGet]
        public async Task<ActionResult> GetAllBranch(string branchId)
        {
            branchId = (string)this.HttpContext.Session["BranchOwerId"];
            var listOfBranchs =await branchServices.GetBranches();
            var listOfBranchss = listOfBranchs.ToList();
      
        
            listOfBranchss.RemoveAll(x => x.Id == branchId || x.Id == "1");
         var data = BuildDropDown(GenerateBranchBranchCode(listOfBranchss));
        ...
        [HttpGet]
        public async Task<ActionResult> GetBranchBankAccount(string accountId)
        {
            
            List<ChartOfAccountStateDto> listOfAccounts = new List<ChartOfAccountStateDto>();
             //listOfAccounts = await _chartOfAccountManagementPositionService.GetAllBranchAccountUsedToCreditCashFlow("560", "3");
            
            var listOfBankAccounts = await _AccountServices.GetAllBranchAccountUsedToCreditCashFlow(accountId);
        ...
        public async Task<ActionResult> AddOrUpdate(CashDemandDataEntity model)
        {
            ...
            else if (model.ServiceOption.Equals("bankCashOutApproval")) //(path == "")
            {
                ...
                model.BankCashOut.BranchId = _accountingEntryServices.GetBranchID();
                model.BankCashOut.BranchCode = _accountingEntryServices.GetBranchCode();
            ...
            {
                model.CashClearing.BranchId = _accountingEntryServices.GetBranchID();
                model.CashClearing.BranchCode = _accountingEntryServices.GetBranchCode();
        ...
        public async Task<ActionResult> InitializeData(string KEY = null, string partialView = null, string path = null)
        {
            ...
            else if (path == "approveDepositNotification")
            {
                ...
                var listBranch =  await branchServices.GetBranches(); 
                // _bankZoneBranchServices.GetAllBranchPresentInZoneByParticipant(OperationEventAttribute.BranchId, "BRANCH");
            ...
            else if (path == "bankDepositCashClearing")
            {
                ...
                else
                {
                    ...
                    mOdelsd.ReferenceId = OperationEventAttribute.Id;
            ...
            else if (path == "FilteringBankDepositOption")
            {
             var DepositNotificationDtos =   new List<DepositNotificationDto>();
                ...
                catch (Exception EX)
                {
                    ...
                    return PartialView(partialView, new CashDemandDataEntity { ListDepositNotificationDto =  new List<DepositNotificationDto>()   });
                }
