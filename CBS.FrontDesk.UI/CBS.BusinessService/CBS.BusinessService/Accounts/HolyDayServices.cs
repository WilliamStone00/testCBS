...
namespace CBS.BusinessService.Accounts
{
    ...
    public class HolyDayServices : BaseService
    {
        private readonly ApiCallerHelper _transactionApiHelper;

        public HolyDayServices()
        {
        ...
        public async Task<ExecutionMessages> Delete(string id)
        {
            try
            {
                var objHolyDay = await GetHolyDay(id);
                var inResponse = await _transactionApiHelper.DeleteAsync<ServiceResponse<bool>>(string.Format(APICallHelper.Get_Update_Delete_HolyDay, id));
                if (inResponse.IsSuccess)
                {

                    GetExecutionMessages(inResponse, true, $"{objHolyDay.EventName}", MessagesResults.Success,
                ...
                else
                {
                    ...
                    GetExecutionMessages(objHolyDay, false, $"{objHolyDay.EventName}", MessagesResults.Failed,
        ...
        public async Task<IEnumerable<HolyDay>> GetHolyDays()
        {
            try
            {
                ...
                var couApiResponse = await _transactionApiHelper.PostAsync<ServiceResponse<List<HolyDay>>>(APICallHelper.GetAllHolyDay, model);
