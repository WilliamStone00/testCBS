...
function ApprovePostedEntriesTransactions(title, message, ajaxUrl, serviceoption, Response, Id, comment) {
    ...
    function PrintDataEntries(response) {
        ...
        if (response) {
            GetPostedEntriesTransactions('Confirm Manual Entry Operation', message, '/ManuallyJournalEntry/PrintCurrentDataEntries', storedId);

        } else {
            GetPostedEntriesTransactions('Confirm Manual Entry Operation', message, '/ManuallyJournalEntry/PrintDataEntries', storedId);
      
        }
  
}

function GetPostedEntriesTransactions(title, message, ajaxUrl, Id) {
    console.log(`URL: ${ajaxUrl}, ID: ${Id}`);

    alertify.confirm(title, message,
        function () {
            // Show loading indicator
            const loading = appalert('Processing and preparing layout for printing ...', 3, 1);// alertify.message('Processing...', 0);

            $.ajax({
                url: ajaxUrl,
                type: 'GET',
                data: { id: Id }, // Note lowercase 'id' to match backend parameter
                success: function (response) {
                  /*  loading.dismiss();*/
                    if (response.success) {
                        openReportPrintingWindow("PrintedManualJE"); // Match backend report name
                        appalert(response.message, 3, 1);
                        // Reasonable reload timeout
...
function GetCurrentPostedEntryTransactions(title, message, ajaxUrl, Id) {
    console.log(`URL: ${ajaxUrl}, ID: ${Id}`);
