...
$(document).ready(function () {
    ...
    $("#manualEntryForm").submit(function (e) {
        ...
        alertify.confirm(
            ...
            "Are you sure you want to save this journal entry?", // Message", // Message
            function () {
                proceedWithManualEntry(); // If confirmed, proceed
            },
            function () {
                alertify.error("Submission Cancelled"); // If cancelled
            }
        ).set({ labels: { ok: "Confirm and Submit", cancel: "Cancel" } });
    });

    function proceedWithManualEntry() {
        let formData = {
            accountingRules: [],
            description: $("#descriptionField").val().trim(),
            accountingEventId: $("#AccountingEventId").val().trim(),
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.Result) {
                    appalert("Entry successfully submitted! " + response.MessageString,1,1);
                } else {
                    appalert("Error submitting entry. " + response.MessageString,1,2);
                }
            ...
            error: function (xhr) {
                appalert("Error submitting entry. Please try again." + xhr.responseText,3,1);
