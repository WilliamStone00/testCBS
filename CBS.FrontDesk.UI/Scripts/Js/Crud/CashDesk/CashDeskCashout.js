...
function PostCashOut() {
    ...
    alertify.confirm("🧾 CONFIRM CASH-OUT OPERATION", message,
        function () {
            ...
            $("#depositorModalTitle").html(`<i class="mdi mdi-account-arrow-right me-2"></i> REPRESENTATIVE INFORMATION REQUIRED`);
            $("#depositorModalDescription").text(`To complete this cash withdrawal, please provide the representative's full identity.`);
            $("#depositorCustomerName").text(memberName);
            $("#depositorCustomerId").text(customerId);

            // ✅ Add Operation Type to the modal if placeholder exists
            if ($("#operationTypeTag").length) {
                $("#operationTypeTag").html(`<span class="badge bg-danger">Normal Withdrawal</span>`);
            }
            ...
            $("#confirmDepositorBtn").off("click").on("click", function () {
                const depositor = collectDepositorInfo();
                if (!validateDepositor(depositor)) {
                    appalert("❗ All representative fields are required to proceed.", 3, 1);
                    return;
                }
...
function PostCashOutSWS() {
    ...
    alertify.confirm("💳 CONFIRM SWS CASH-OUT OPERATION", message,
        function () {
            ...
            $("#depositorModalTitle").html(`<i class="mdi mdi-account-arrow-right me-2"></i> REPRESENTATIVE INFORMATION REQUIRED`);
            $("#depositorModalDescription").text(`Please fill in the representative's full information to finalize this transaction.`);

            // ✅ Inject operation type tag (if tag element exists)
            if ($("#operationTypeTag").length) {
                $("#operationTypeTag").html(`<span class="badge bg-warning text-dark">SWS Withdrawal</span>`);
            }
            ...
            $('#myDataTableT tbody tr').each(function () {
                ...
                if (amount > 0 || fee > 0) {
                    accountDetails += `\n- ${accountType}: Amount ${amount.toLocaleString('en-US')} FCFA, Fee ${fee.toLocaleString('en-US')} FCFA`;
                }
            });

            // Construct default note
            const representativeNote = `
            SWS Cash-out performed by authorized representative.
            👤 Member: ${memberName} (${customerId})
            💵 Total Amount: ${totalInfo.total.toLocaleString('en-US')} FCFA
            📄 Accounts Involved:${accountDetails}
            ✅ Operation complies with audit and withdrawal notification policies.
            `.trim();
            // Inject default note
            setTimeout(() => {
                $("#depositerModal").modal("show");
                $("#RepresentativeNote").val(representativeNote);
            }, 100);
...
function GetMember() {
    ...
    // Handle empty input
    if (!memberId) {
        appalert("⚠️ Please enter a valid member account number before proceeding.", 3, 1);
        return;
    }

    // Check if it's a provisional (PRM) member
    if (memberId.includes("PRM")) {
        appalert(`
            🚫 <strong>Provisional Member Detected</strong><br/>
            This account belongs to a <strong>Pre-Registered Member (PRM)</strong> and is <u>not yet eligible</u> for withdrawal operations.
            <br/><br/>
            <strong>Why?</strong><br/>
            PRM members have not completed their membership onboarding and are still in provisional status.
            <br/><br/>
            <strong>How to Resolve:</strong>
            <ul style="margin-bottom:0;">
                <li>Proceed to the <strong>Cash Desk</strong> and initiate a <strong>Membership Deposit (Cash-In)</strong>.</li>
                <li>Ensure all required onboarding items (e.g. Membership Fee, Share Contribution) are paid in full.</li>
                <li>Once completed, the system will automatically upgrade the member to <strong>Active</strong> status and generate a permanent Member Account Number.</li>
                <li>After that, all withdrawal and financial operations will be available.</li>
            </ul>
        `, 3, 1);
        console.warn("❌ Withdrawal blocked: PRM members require cash-in for full registration.");
        return;
    }

    // Default operation for active members
    const operation = "cashout";
    console.log("✅ Cash-out Mode Activated");

    // Save to hidden field for reference
