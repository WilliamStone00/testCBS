...
function ReadExcelFile() {
    ...
    formData.append("CollectorId", branchCode);
 

    event.preventDefault();
    updateProgressBar(0);
    $('#progressBarContainer').show();
    var messageStatus = "Are you sure you all the data inputed are correct and ready to procceed with operations?"
 
    alertify.confirm("T R U S T S O F T C R E D I T Daily Collection Centralization", messageStatus,
        function () {
            // If the user confirms, proceed with the submission
            $.ajax({
                url: "/DailyAgentManagement/UploadAccountModel/",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
