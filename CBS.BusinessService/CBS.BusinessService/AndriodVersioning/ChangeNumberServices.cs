...
namespace CBS.BusinessService.AndriodVersioning
{
    public class ChangeNumberServices : BaseService
    {
        ...
        public async Task<ExecutionMessages> Create(ChangePhoneNumberCommand model)
        {
            try
            {
        ...
        public async Task<ExecutionMessages> Update(ApprovePhoneNumberRequestCommand model)
        {
            try
            {
                var response = await _customerConfigApiHelper.PostAsync<ServiceResponse<bool>>(APICallHelper.ApprovePhoneNumberRequest, model);
                if (response.ApiResponseData != null)
                {
                    // Successful creation
                    GetExecutionMessages(response, true, null, MessagesResults.Success,
