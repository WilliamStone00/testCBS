...
{
    public class TrialBalanceService : BaseService
    {
        ...
        /// </summary>
        public async Task<TrialBalanceResponseDto> GetTrialBalancesAsync6columns(TrialBalanceFilterQuery filter)
        {
            try
            {
               
                // POST request to the API
                var response = await _apiCallerHelper.PostAsync<ServiceResponse<List<TrialBalanceDto>>>(
                ...
                {
                    result.Lines = response.ApiResponseData?.Data ?? new List<TrialBalanceDto>();
                }
                else
                {
                    result.Lines = new List<TrialBalanceDto>();
        ...
        public async Task<TrialBalanceResponseDto> GetTrialMockInformation()
        {
